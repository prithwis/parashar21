...
def R601_GenerateLLMInput03(filename="LLM_Input_Verbose.txt"):
...
# -------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------
def R600_GenerateLLMInput02(filename="LLM_Input_Verbose.txt"):
    ...
    # p21.Lord is 1-based. Index 0 is dummy.
    for house in range(1, len(p21.Lord)):
    ...
    for graha in grahaOrder:
        ...
        if graha in p21.GRashiN:
            ...
            add(
                f"- Located in Rashi {p21.GRashiN[graha]}"
            )
    ...
    for bhav in sorted(p21.BAspectedBy2.keys()):

        grahas = p21.BAspectedBy2[bhav]

        add(
            f"Bhava {bhav} receives aspect from {GList(grahas)}"
        )
    ...
    for bhav in sorted(p21.BAspectedByBL2.keys()):

        lords = p21.BAspectedByBL2[bhav]

        add(
            f"Bhava {bhav} receives aspect from Lords {lords}"
        )
...
def R600_GenerateLLMInput(filename="LLM_Input.txt"):
    ...
    for graha in p21.GrahaBhava.keys():
    ...
    for graha, sources in p21.GAspectedBy2.items():
        ...
        add(
            f"{graha} is aspected by {sources}"
        )
    ...
    for lord, grahas in p21.BLConjunctsG2.items():
        ...
        add(
            f"Lord of Bhava {lord} conjuncts {grahas}"
        )
    ...
    for bhav in sorted(p21.BAspectedBy2.keys()):
        ...
        add(
            f"Bhava {bhav} receives aspect from {grahas}"
        )
